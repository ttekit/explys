      ...
      <div className="relative max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          {featured ? (
            ...
            <span className="text-sm text-muted-foreground">{t.browseBelow}</span>
          )}
        </div>

        <h1 className="font-lexend mb-4 text-balance text-4xl font-bold sm:text-5xl lg:text-6xl">
          {featured ? featured.title : t.titleFallback}
        </h1>

        <p className="mb-8 max-w-2xl text-lg leading-relaxed text-muted-foreground">
          {featured ? featured.description : t.descriptionFallback}
        </p>

        <div className="mb-8 flex flex-wrap gap-4 text-sm text-muted-foreground">
          <span>{t.metaFresh}</span>
          <span className="flex h-1 w-1 items-center justify-center">
            <span className="mt-4 h-1 w-1 rounded-full bg-muted-foreground" />
          </span>
          <span>{t.metaQuizzes}</span>
          <span className="flex h-1 w-1 items-center justify-center">
            <span className="mt-4 h-1 w-1 rounded-full bg-muted-foreground" />
          </span>
          <span>{t.metaLevels}</span>
        </div>
